...
const perfil = (id: string, nombre: string, extra: Partial<Raw> = {}): Perfil => aPerfil({
  ...
  equipos: { name: 'Clientes' }, ...extra,
})
const mapa = (...ps: Perfil[]) => new Map(ps.map((p) => [p.persona.id, p]))

test('aPerfil: posición y equipo (objeto o lista), día de alta en Madrid y estado', () => {
  const a = perfil('u1', 'Laura Gómez', {
    created_at: '2026-09-13T23:30:00Z', positions: [{ name: 'SEO Strategist' }], equipos: [{ name: 'RRHH' }], status: 'inactivo',
  })
  expect(a.persona).toEqual({ id: 'u1', nombre: 'Laura Gómez', email: 'u1@ejemplo.com', posicion: 'SEO Strategist', equipo: 'RRHH', rol: 'manager' })
  ...
  // Nadie tiene equipo hasta que el admin lo asigne (0049): null es el caso normal al
  // principio, no un error, y el aviso tiene que salir igual.
  expect(perfil('u3', 'Sin Equipo', { equipos: null }).persona.equipo).toBeNull()
})

test('managerDe: el manager directo si existe', () => {
  const jefe = perfil('m1', 'Carlos Ruiz', { equipos: { name: 'RRHH' } })
  const laura = perfil('u1', 'Laura Gómez', { manager_id: 'm1' })
  const todos = mapa(jefe, laura)
  // El manager lleva SU equipo, no el de la persona avisada: es a él a quien hay que
  // enrutar el mensaje.
  expect(managerDe(laura, todos)).toEqual({ id: 'm1', nombre: 'Carlos Ruiz', email: 'm1@ejemplo.com', equipo: 'RRHH' })
...
test('managerPorNombre: sin mayúsculas ni espacios; ambiguo o desconocido sin email', () => {
  ...
  expect(managerPorNombre('  carlos ruiz ', todos)).toEqual({ id: 'm1', nombre: 'Carlos Ruiz', email: 'm1@ejemplo.com', equipo: 'Clientes' })
  // Ambiguo o desconocido: el equipo va a null porque no se sabe de quién es, igual que
  // el email. Inventar uno sería peor que no mandarlo.
  expect(managerPorNombre('Ana Pérez', todos)).toEqual({ id: null, nombre: 'Ana Pérez', email: null, equipo: null })
  expect(managerPorNombre('Pilar', todos)).toEqual({ id: null, nombre: 'Pilar', email: null, equipo: null })
