-- 0013_horas_area_role_scope.sql
-- La restricción de área-asignada aplica SOLO a operativos; manager/admin
-- pueden registrar contra cualquier área. Además el área "Interno" queda
-- reservada al proyecto "Departamento" (no se permite en proyectos de cliente).
...
begin
  ...
  for v_line in select * from jsonb_array_elements(p_lines) loop
    ...
    if btrim(v_line->>'project') = 'Departamento' then
      -- "Departamento" debe usar el área interna.
      if (v_line->>'area_id')::uuid <> v_internal_area_id then
        raise exception 'el proyecto Departamento debe usar el área interna';
      end if;
    else
      -- Proyectos de cliente: nunca usan el área interna.
      if (v_line->>'area_id')::uuid = v_internal_area_id then
        raise exception 'el área interna solo aplica al proyecto Departamento';
      end if;
      -- Solo los operativos están restringidos a sus áreas asignadas;
      -- manager/admin pueden registrar contra cualquier área.
