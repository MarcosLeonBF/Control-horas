-- 0027_horas_descripcion_general_departamento.sql
-- Las descripciones del proyecto "Departamento" son una LISTA GENERAL (compartida por
-- todos los departamentos), no una por departamento.
--
-- Cambio respecto a 0025/0026:
--   · Antes: en "Departamento" la descripción debía pertenecer a departamento_descripciones
--     del departamento de la línea.
--   · Ahora: en "Departamento" la descripción debe estar en la lista general de
--     descripciones ACTIVAS (public.descripciones, active = true). No depende del departamento.
--   · Proyecto cliente: sigue siendo texto libre (solo no vacía).
--
-- Se elimina la tabla departamento_descripciones (ya sin uso).
...
begin
  ...
  for v_line in select value from jsonb_array_elements(p_lines) loop
    ...
    if btrim(v_line->>'project') = 'Departamento' then
      ...
      -- Descripción del proyecto "Departamento": lista general (descripciones activas).
      if not exists (
...
drop table if exists public.departamento_descripciones;
