-- ============================================================
-- 0031 HORAS: el chequeo de "líneas duplicadas" incluye la descripción
-- ------------------------------------------------------------
-- Bug: guardar_registro consideraba duplicadas dos líneas con el mismo
-- (entry_date, project, area_id, department, etapa_id) aunque tuvieran DISTINTA
-- descripción. En "Departamento" la etapa la fija el departamento, así que dos
-- líneas del mismo departamento (p. ej. "Todos") con descripciones distintas
-- ("Reportes…" y "Mejoras…") colisionaban falsamente → "hay líneas duplicadas".
-- Fix: agregar la descripción (btrim) al agrupamiento del chequeo de duplicados.
-- Se re-crea la función completa (misma definición viva 0030) con esa única línea
-- cambiada.
-- ============================================================
...
begin
  ...
  -- La descripción se incluye para no colisionar líneas de "Departamento" del mismo
  -- departamento (misma etapa) con textos distintos.
  if exists (
    select 1 from (
      select e->>'entry_date' dt, e->>'project' p, e->>'area_id' a, e->>'department' d, e->>'etapa_id' et, btrim(e->>'description') ds
